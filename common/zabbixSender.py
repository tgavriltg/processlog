...
class ZSend:
    def __init__(self,logging,server=ZABBIX_SERVER,port=ZABBIX_PORT):
  ...
  "clock":%s }'''
    ...
    def print_vals(self):
        for (h,k,v,t1) in self.list:
            self.logging.info("Host: %s, Key: %s, Value: %s, Event: %s" % (h,k,v,t1))
    ...
    def send(self,mydata):
        ...
        try:
            ...
            self.logging.error("Error talking to server: %s\n" % err)
            return (255,err)

        response_header = sock.recv(5)
        if not response_header == 'ZBXD\1':
            self.logging.error("Invalid response from server." + \
                            "Malformed data?\n---\n%s\n---\n" % mydata)
            return (254,err)
        ...
        match = re.match("^.*failed:\s(\d+).*$",str(response))
        if match is None:
            self.logging.error("Unable to parse server response - " + \
                            "\n%s\n" % response)
        else:
            fails = int(match.group(1))
            if fails > 0:
                self.logging.info("Failures reported by zabbix when sending:" + \
                               "\n%s\n" % mydata)
                return (1,response)
        return (0,response)
